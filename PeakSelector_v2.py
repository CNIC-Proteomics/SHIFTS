...
def modelate_threshold(maximum_slopes,ci_removal):
    ...
    # outfile=args.infile[:-15]+'_to_modelate.tsv'
    
    # to_modelate.to_csv(outfile,sep='\t',index=False)
...
def plotfitting(curve, to_modelate, ci_removal):
    ...
    outfile_plot=args.infile[:-15] + 'fit_plot.html'
...
def peakSelector(df_hist, curve,frequency, apex_points, decimal_places):
    ...
    outfile=args.infile[:-15] + 'DMHistogram_threshold.tsv'
